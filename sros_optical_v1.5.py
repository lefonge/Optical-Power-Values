            ...
            if len(parts) >= 4:
                try:
                    ...
                    if re.match(r'[-\d\.]+', tx_power) and re.match(r'[-\d\.]+', rx_power):
                        data.append([vendor, f"{port}/Lane{lane_id}", tx_power, rx_power])
                    else:
        ...
        if "Tx Output Power (dBm)" in line:
            try:
                ...
                if tx_match and rx_match:
                    ...
                    data.append([vendor, port, tx_power, rx_power])
            except IndexError:
...
def save_to_csv(data, output_file):
    with open(output_file, mode='w', newline='') as file:
        ...
        writer.writerow(["Vendor", "Port", "Tx Power (dBm)", "Rx Power (dBm)"])
